...
function handleContactInput(answer) {
    if (answer.toLowerCase() === "back") {
        return;
    }
    ...
    switch (choice) {
        ...
        case "yes":
            ...
            return;
        default:
            console.log("Invalid choice. You can choose to 'exit' or type: 'yes' or 'no'.");
            rl.prompt();
            return;
    }
}
        ...
        rl.question("Mail: ", (mail) => {
            rl.question("Phonenumber: ", (phoneNumber) => {
                rl.question("Preferred Contact Method: ", (contactMethod) => {
                    ...
                    console.log(`We'll reach out to you ${newUser.userInfo.name} with a expert on the area in ${inputs.subtopic}`);
                    rl.close();
                });
            });
        });
